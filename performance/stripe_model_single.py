...
parser = argparse.ArgumentParser(
    description="Test performance of stripe model" " per label on example graphs."
)
parser.add_argument(
    "--quick", action="store_true", help="Perform only on smaller graph."
)
...
for filename in os.listdir("data/"):
    ...
    if not np.allclose(
        model.expected_num_edges(get=True), g.num_edges, atol=args.tol, rtol=0
    ):
        print("Distance from root: ", model.exp_num_edges - g.num_edges)
    ...
    for i in range(g.num_labels):
        if not np.allclose(
            np.sum(out_deg[out_deg.label == i].value),
