...
class DiGraphEnsemble(GraphEnsemble):
    ...
    def log_likelihood(self, g, selfloops=None):
        ...
        like = self._likelihood(
            ...
            self.prop_dyad, adj, self.selfloops)
...
class RandomDiGraph(DiGraphEnsemble):
    ...
    def log_likelihood(self, g, selfloops=None):
        ...
        if selfloops:
            like += (self.num_vertices**2 - np.count_nonzero(adj)
                     ) * log1p(-self.param[0])
        else:
            like += (self.num_vertices*(self.num_vertices - 1) - 
                     np.count_nonzero(adj)) * log1p(-self.param[0])
...
class MultiDiGraphEnsemble(DiGraphEnsemble):
    ...
    def expected_num_edges_label(self, recompute=False):
        ...
        if not hasattr(self, '_exp_num_edges_label') or recompute:
            self._exp_num_edges_label = self.exp_edges_label(
                self.p_ijk, self.param, self.prop_out, self.prop_in, 
                self.prop_dyad, self.selfloops)
    ...
    def expected_degree_by_label(self, recompute=False):
        ...
        if not hasattr(self, '_exp_degree_label') or recompute:
            res = self.exp_degrees_label(self.p_ijk, self.param, self.prop_out,
                                         self.prop_in, self.prop_dyad, 
    ...
    def log_likelihood(self, g, selfloops=None):
        ...
        if tensor:
            like = self._likelihood_tensor(
                self.logp_ijk, self.log1mp_ijk, self.param, self.prop_out,
                self.prop_in, self.prop_dyad, adj, selfloops)
        else:
            like = self._likelihood(
                self.logp, self.log1mp, self.param, self.prop_out, 
                self.prop_in, self.prop_dyad, adj, selfloops)
    ...
    def sample(self, ref_g=None, weights=None, out_strength_label=None, 
        ...
        if weights is None:
            ...
            lbls, rows, cols, vals = self._cremb_sample_tensor(
                self.p_ijk, self.param, self.prop_out, self.prop_in,
                self.prop_dyad, s_out_l, s_in_l, self.selfloops)
        else:
            raise ValueError(
    ...
    @staticmethod
    @jit(nopython=True)  # pragma: no cover
    def exp_edges_label(p_ijk, param, prop_out, prop_in, 
                        prop_dyad, selfloops):
    ...
    @staticmethod
    @jit(nopython=True)  # pragma: no cover
    def exp_degrees_label(p_ijk, param, prop_out, prop_in, prop_dyad, 
                          num_v, num_l, selfloops):
        """ Compute the expected undirected, in and out degree sequences.
        """
        ...
        for lbl in range(num_l):
            for i in range(num_v):
                ...
                for j in range(i + 1):
                    ...
                    if i != j:
                        pij = p_ijk(param[lbl], p_out_i, p_in_j, 
                                    prop_dyad(i, j))
                        pji = p_ijk(param[lbl], p_out_j, p_in_i, 
                                    prop_dyad(j, i))
                        p = pij + pji - pij*pji
                        ...
                        pii = p_ijk(param[lbl], p_out_i, p_in_j, 
                                    prop_dyad(i, j))
                        exp_d[i, lbl] += pii
                        exp_d_out[i, lbl] += pii
                        exp_d_in[i, lbl] += pii
    ...
    @staticmethod
    @jit(nopython=True)  # pragma: no cover
    def _likelihood_tensor(logp_ijk, log1mp_ijk, param, prop_out, prop_in,
                           prop_dyad, adj, selfloops):
        ...
        for k in range(adj.shape[0]):
            for i, p_out_i in enumerate(prop_out[:, k]):
                for j, p_in_j in enumerate(prop_in[:, k]):
                    if (i != j) | selfloops:
                        if adj[k, i, j] != 0:
                            tmp = logp_ijk(param[k], p_out_i, p_in_j, 
                                           prop_dyad(i, j))
                        else:
                            tmp = log1mp_ijk(param[k], p_out_i, p_in_j, 
                                             prop_dyad(i, j))
    ...
    @staticmethod
    @jit(nopython=True)  # pragma: no cover
    def _binary_sample_tensor(p_ijk, param, prop_out, prop_in, prop_dyad, 
                              selfloops):
        """ Sample from the ensemble.
        """
        lbls = List()
    ...
    @staticmethod
    @jit(nopython=True)  # pragma: no cover
    def _cremb_sample_layer(p_ijk, param, prop_out, prop_in, prop_dyad, 
                            s_out, s_in, selfloops):
        """ Sample from the ensemble with weights from the CremB model.
        """
